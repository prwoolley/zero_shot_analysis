...
pkls = ['../../outputs/logits/proteingym/ESM3_sm_open_v0.both.logits.x.combined.pkl',
        '../../outputs/logits/proteingym/protein_mpnn.proteinmpnn_v_48_020.logits.x.combined.use_sequence0.pkl',
        '../../outputs/logits/proteingym/esmc_600m.logits.x.combined.pkl']


for i, model in enumerate(models):
    PROTEINGYMLOGITPKL = pkls[i]
    print(f"Processing model: {model}")
    print(f"Using logits from: {PROTEINGYMLOGITPKL}")
    ...
    def tidy_fitclass_density_data(model):
        ...
        fitclass_rho.to_csv(outdir / f'figure4_fitclass_density_{model}.csv', index=False)

    tidy_fitclass_density_data(model)