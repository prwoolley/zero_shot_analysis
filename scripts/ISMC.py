...
def load_model(model_choice, config, device):
    esmc_models = [
        'esmc_600m',
        'esmc_300m'
        ]
    print('Loading ISM model...')
    if model_choice not in esmc_models:
        print(f"Invalid model choice: {model_choice}. Using default of esmc_600m instead...")
        model_choice = "esmc_600m"
    device = torch.device(f"cuda:{device}" if torch.cuda.is_available() else "cpu")
    model = ESMC.from_pretrained(model_choice).to(device)
...
def process_sequences(model,tokenizer,device,sequences,indices,mode): # forward pass
    tokens = tokenizer.batch_encode_plus(sequences,padding=True,return_tensors='pt')
    tokens = {k: v.to(device) for k, v in tokens.items()}
    with torch.no_grad():
        output = model.forward(tokens['input_ids'],tokens['attention_mask'])
    if mode == 'embeds':
        output = output.embeddings.float().cpu().numpy().astype(np.float128)
        return [output[i,j] for i, j in enumerate(indices)]
    else:
        output = output.sequence_logits.float().cpu().numpy().astype(np.float128)
        return [output[i,j] for i, j in enumerate(indices)]
...
def main(args):
    ...
    model, tokenizer, device = load_model(model_choice, config, device)
    ...
    for i in range(chkpt_num):
        ...
        for j in pop_j[::-1]:
            try:
                ...
                continue
        config = config.split('/')[-1].split('.pth')[0] # using the basename of the config path
        names,as_pkl_dict = prep_names(headers_chunk, operations, config, mode, indices_str_chunk, as_pkl+chkpt_str)
        save_outputs(outputs,output_dir,names,as_pkl_dict)
    return        

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process sequences using ISM model.')
    parser.add_argument('--fasta', type=str, help='Input FASTA file')
    parser.add_argument('--config', type=str, help='Path to local .pth file')
    parser.add_argument('--model_choice', type=str, help='Model to use. Options [esmc_600m, esmc_300m].', default = 'esmc_600m')
