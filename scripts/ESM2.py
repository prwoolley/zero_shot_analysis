...
def load_model(model_choice, device):
    ...
    if model_choice not in esm2_models:
        print(f"Invalid model choice: {model_choice}. Using default of esm2_t33_650M_UR50D instead...")
        model_choice = 'esm2_t33_650M_UR50D'
    tokenizer = AutoTokenizer.from_pretrained(f"facebook/{model_choice}")
    model = AutoModelForMaskedLM.from_pretrained(f"facebook/{model_choice}")
