...
def load_model(model_choice, device):
    ...
    if model_choice not in saprot_models:
        print(f"Invalid model choice: {model_choice}. Using default of SaProt_650M_PDB instead...")
        model_choice = 'SaProt_650M_PDB'
    device = torch.device(f"cuda:{device}" if torch.cuda.is_available() else "cpu")
    tokenizer = AutoTokenizer.from_pretrained(f"westlake-repl/{model_choice}")
    model = AutoModelForMaskedLM.from_pretrained(f"westlake-repl/{model_choice}").to(device)
...
def read_fasta(fasta_file,isfoldseek=False):
    ...
    with open(fasta_file, 'r') as inf:
        ...
        if header and seq:
            fasta[header] = seq
    #if isfoldseek==True:
    #    fasta = {x.split('.pdb')[1]:y for x,y in fasta.items()}
    return fasta

def combine_fasta(aa_fasta,fs_fasta):
    aa_fasta = {x:y for x,y in aa_fasta.items() if x in fs_fasta.keys()}
    fs_fasta = {x:y for x,y in fs_fasta.items() if x in aa_fasta.keys()}
    headers = []
    sequences = []
    for key in aa_fasta.keys():
        if len(aa_fasta[key]) != len(fs_fasta[key]):
            continue
        else:
            headers.append(key)
            sequences.append(''.join([aa_fasta[key][i]+fs_fasta[key][i].lower() for i in range(len(aa_fasta[key]))]))
...
def prep_seqs(amino_acids, foldseek, indices_str):
    aa_fasta = read_fasta(amino_acids,isfoldseek=False)
    fs_fasta = read_fasta(foldseek,isfoldseek=True)
    headers,sequences = combine_fasta(aa_fasta,fs_fasta)
...
def sequence_masking(sequences, tokenizer, indices, mask_strategy = 'esm'): # mask sequences for MLM loop
    """
    mask_strategy "esm" = <mask>, replaces amino acid and foldseek token with ESM mask token
    mask_strategy "saprot_aa" = #x, replaces amino acid but not foldseek token
    mask_strategy "saprot_fs" = X#, replaces foldseek token but not amino acid
    mask_strategy "saprot_both" = ##, replaces both amino acid and foldseek token with saprot mask token
    """
    masked_sequences = []
    for i,sequence in enumerate(sequences):
        if mask_strategy == 'esm':
            masked_sequence = [f'{sequence[:j*2-2]}{tokenizer.mask_token}{sequence[j*2:]}' for j in indices[i]]
        elif mask_strategy == 'saprot_aa':
            masked_sequence = [f'{sequence[:j*2-2]}#{sequence[j*2-1:]}' for j in indices]
        elif mask_strategy == 'saprot_fs':
            masked_sequence = [f'{sequence[:j*2-1]}#{sequence[j*2:]}' for j in indices]
        elif mask_strategy == 'saprot_both':
            masked_sequence = [f'{sequence[:j*2-2]}##{sequence[j*2:]}' for j in indices]
        masked_sequences.append(masked_sequence)
    return masked_sequences

def prep_mlm(sequences,indices,tokenizer,mask_strategy):
    all_sequences = []
    all_indices = []
    masked_sequences = sequence_masking(sequences,tokenizer,indices,mask_strategy)
...
def batch_run(model, tokenizer, sequences, indices_str, device, batch_size, mode, operations): # batch sequences, process, and concatenate
    ...
    while sequence_index < len(sequences):
        try:
            ...
            batch_indices = [index_str_to_arrays(index, len(seq)//2) for index,seq in zip(indices_str[sequence_index:sequence_index+batch_size],batch_sequences)]
...
def mlm_batch_run(model, tokenizer, sequences, indices_str, mask_strategy, device, batch_size, mode, operations): # batch sequences, mask, process, and concatenate
    ...
    while (finish == False):
        ...
        while batch_len < batch_size: # Add sequences to batch until batch_size is reached
            if batch_sequence_index == (len(sequences)):
                last_seq = True
                break
            curr_index = index_str_to_arrays(indices_str[batch_sequence_index], len(sequences[batch_sequence_index])//2)
        ...
        flatten_sequences,flatten_indices = prep_mlm(batch_sequences,batch_indices,tokenizer,mask_strategy)
        try:
...
def main(args):
    ...
    fasta_foldseek = args.fasta_foldseek
    ...
    headers,sequences,indices_str = prep_seqs(fasta_aa,fasta_foldseek,indices_str)
    ...
    for i in range(chkpt_num):
        ...
        if mlm_loop == True:
            ...
            outputs,batch_size = mlm_batch_run(model, tokenizer, sequences_chunk, indices_str_chunk, mask_strategy, device, mlm_batch_size, mode, operations)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--fasta_aa', type=str, help='Path to the amino acid fasta file')
    parser.add_argument('--fasta_foldseek', type=str, help='Path to the foldseek fasta file')
    parser.add_argument('--model_choice', type=str, help='Choice of SaProt model. Options: [SaProt_35M_AF2, SaProt_650M_PDB, SaProt_650M_AF2] Default: SaProt_650M_PDB', default = 'SaProt_650M_PDB')
    ...
    parser.add_argument('--mask_strategy', type=str, help='Masking strategy for MLM loop. Options: [esm, saprot_aa, saprot_fs, saprot_both]. Default: esm', default = 'esm')
