...
def index_str_to_arrays(indices_str, seq_len): # convert string of indices to list of indices
    ...
    return list(set(prep_indices))


def sequence_masking(sequences, indices):
    masked_sequences = []
    for i,sequence in enumerate(sequences):
        masked_sequence = [f'{sequence[:j*2-2]}[MASK] {sequence[j*2:]}' for j in indices[i]]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process sequences using model.')
    ...
    parser.add_argument('--mlm_batch_size', type=int, help='Batch size for MLM loop. Default: 1000', default = 1000)
